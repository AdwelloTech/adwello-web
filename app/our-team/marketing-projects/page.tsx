...
export default async function MarketingProjectsPage() {
  let marketingProjects: MarketingProject[] = [];

  try {
    marketingProjects = await getAllMarketingProjects();
  } catch (error) {
              ...
              {marketingProjects.length > 0 ? (
                <div className="flex justify-center">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {marketingProjects.map((project) => (
                      <Link
                        key={project._id}
                        href={`/our-team/marketing-projects/${project.detailPage}`}
                        className="block"
                      >
                ...
                <div className="text-center text-white py-12">
                  <p className="text-lg mb-4">
                    No marketing projects available yet.
                  </p>
                  <p className="text-gray-400">
                    Create marketing projects in Sanity Studio to see them here.
                  </p>
                </div>
              )}
            </div>
